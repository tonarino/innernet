...
use crate::{
    ...
    Context, ServerError, Session, VERSION,
};
...
pub async fn routes(
    ...
    match (req.method(), components.pop_front().as_deref()) {
        ...
        (&Method::GET, Some("info")) => handlers::info().await,
...
mod handlers {
    ...
    pub async fn info() -> Result<Response<Body>, ServerError> {
        let version = Version::parse(VERSION).expect("Server version can be parsed");

        json_response(Info { version })
    }
